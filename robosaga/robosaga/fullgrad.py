...
class FullGrad:
    ...
    Modified based on the original code from the FullGrad repository:
    ...
    def fullGradientDecompose(self, x, net_input=None):
        ...
        input_gradient, feature_gradients = self.model_ext.get_feature_grads(x, output_scalar)
        ...
        for i in range(L):
            # feature gradients are indexed backwards because of backprop
    ...
    def __call__(self, image, net_input=None):
        # FullGrad saliency
        self.model.eval()
        input_grad, bias_grad = self.fullGradientDecompose(image, net_input)

        with torch.no_grad():
            # Initailize CAM with input gradients * image
            cam = self._postProcess(input_grad * image).sum(1, keepdim=True)

            im_size = image.size()
            H, W = im_size[2], im_size[3]
            # Aggregate Bias-gradients
            for i in range(len(bias_grad)):
                # Select only Conv layers
                if len(bias_grad[i].size()) == len(im_size):
                    # different from the original code, summing channel-wise before interpolation, to accelerate computation
