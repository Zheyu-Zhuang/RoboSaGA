...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    def __init__(
        ...
        **kwargs,
        ...
        with config.unlocked():
            ...
            if robomimic_visual_encoder is not None:
                config.observation.encoder.rgb.core_kwargs.pool_class = robomimic_visual_encoder[
                    "pool_class"
                ]
    ...
    def compute_loss(self, batch, epoch_idx=None, batch_idx=None, robosaga=None):
        # normalize input
        assert "valid_mask" not in batch
        nobs = self.normalizer.normalize(batch["obs"]) if self.normalize_obs else batch["obs"]
        nactions = self.normalizer["action"].normalize(batch["action"])
        global_ids = batch["ids"].flatten()
