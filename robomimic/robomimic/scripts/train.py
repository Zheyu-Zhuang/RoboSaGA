...
def train(config, device):
    ...
    if ckpt is not None:
        model, _, _ = FileUtils.resume_from_checkpoint(
            ckpt_path=ckpt,
            device=device,
        )
    ...
    # SaGA: making randomizer consistent with Diffusion policy
    replace_submodules(
    ...
    if "saliency" in config and config.saliency.enabled:
        ...
        saga = RoboSaGA(model.nets["policy"], **config.saliency)
    ...
    for epoch in range(1, config.train.num_epochs + 1):  # epoch numbers start at 1
        ...
        if config.experiment.validate:
            model.nets["policy"].disable_low_noise = True
            # with torch.no_grad():
            step_log = TrainUtils.run_epoch(
                model=model,
