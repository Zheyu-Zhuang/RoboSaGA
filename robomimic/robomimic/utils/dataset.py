...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def load_demo_info(self, filter_by_attribute=None, demos=None):
        ...
        if demos is not None:
            ...
            self.demos = [
                elem.decode("utf-8")
                for elem in np.array(self.hdf5_file["mask/{}".format(filter_by_attribute)][:])
            ]
        else:
        ...
        if self.enable_color_jitter:
            for k in meta["obs"]:
                ...
                if im_type != "image":
                    continue
                out_images = []
                n_images = meta["obs"][k].shape[0]
                for i in range(n_images):
                    im_ = meta["obs"][k][i]
                    out_images.append(self.color_jitter(im_))
                meta["obs"][k] = np.stack(out_images, axis=0)
